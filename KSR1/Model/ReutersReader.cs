    ...
    public static class ReutersReader
    {
        public static IEnumerable<ReutersMetricObject> ReadReuters(string filepath)
        {
            ...
            var ret = document.DocumentNode.Descendants("REUTERS")
                .Where(r => r.Descendants("PLACES").First().Descendants("D").Any(d => places.Contains(d.InnerText)))
                .Select(
                    r => new ReutersMetricObject(
                        r.Descendants("PLACES").First().Descendants("D").Select(d => System.Net.WebUtility.HtmlDecode(d.InnerText)),
